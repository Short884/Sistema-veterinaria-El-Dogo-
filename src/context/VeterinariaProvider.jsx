...
export const VeterinariaProvider = ({ children }) => {
  ...
  // datosActualizados puede ser un objeto plano o un FormData (cuando incluye foto nueva)
  const actualizarCliente = async (id, datosActualizados) => {
    try {
      // El backend documenta PATCH (no PUT) para edición
      const respuesta = await api.patch(`/clientes/${id}`, datosActualizados);
      const listaActualizada = clientes.map(cliente =>
        cliente.id === id ? respuesta.data : cliente
      );
      setClientes(listaActualizada);
    } catch (error) {
  ...
  // datosActualizados puede ser un objeto plano o un FormData (cuando incluye foto nueva)
  const actualizarMascota = async (id, datosActualizados) => {
    try {
      const respuesta = await api.patch(`/mascotas/${id}`, datosActualizados);
      const listaActualizada = mascotas.map((mascota) =>
        mascota.id === id ? respuesta.data : mascota
      );
      setMascotas(listaActualizada);
    } catch (error) {
